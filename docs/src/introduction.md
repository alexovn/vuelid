...
- **Choose validation mode**: `lazy` (or after-submit) mode allows users to fill out the entire form before initiate its validation, while `eager` mode triggers validation process as soon as the users start typing.
- **Get errors by field path**: It's very useful for displaying UX-friendly error messages under the fields.
